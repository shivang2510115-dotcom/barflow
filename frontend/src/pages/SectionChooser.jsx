...
export default function SectionChooser() {
  ...
  if (!property) {
    return (
      <div className="p-8 md:p-12 text-faint font-mono text-xs uppercase tracking-widest">
        Loading…
      </div>
    );
  }
  ...
  if (sections.length === 0) {
    return (
      <div className="p-8 md:p-12">
        <div className="text-[10px] uppercase tracking-[0.4em] font-mono text-brass mb-3">
        ...
        <p className="text-muted2 mt-4 max-w-lg">
          ...
          screens on <span className="text-ink">Admin → Staff</span> — ask whoever
  ...
  return (
    <div className="p-6 md:p-10">
      ...
      <p className="text-muted2 mb-10 max-w-xl">
        ...
        {sections.map((s) => {
          ...
          return (
            <button
              ...
              className="group text-left border border-hairline bg-surface rounded p-6 hover:border-brass/60 transition-colors"
            >
              {Icon && <Icon className="w-7 h-7 text-brass mb-4" strokeWidth={1.5} />}
              ...
              <div className="mt-4 min-h-[3.25rem]">
                {figure && (
                  <>
                    ...
                    <div className="text-[11px] tracking-widest uppercase text-faint mt-1">
                      {figure.sub}
                    </div>
                  </>
                )}
              </div>
              <p className="text-sm text-muted2 mt-2">{blurb}</p>
            </button>
          );
        })}
      </div>
